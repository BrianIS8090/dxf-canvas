...
Исправлен расчёт деталей с разгрузочными прорезями для гибки. Площадь берётся по замкнутым границам с вычитанием отверстий; в P дополнительно входит длина прорезей, каждая линия один раз. Их количество и суммарная длина указаны отдельной строкой измерения. [Подробности и ограничения](docs/releases/v0.8.1.md).
...
В 0.8.1 локально прошли 133 публичных теста и отдельная проверка площади/длины реза на исходном DXF с виртуального диска I: без изменения его содержимого. Загрузка, подсветка прорезей и размещение подписи также проверены в интерфейсе. В 0.8.0 дополнительно проверялись четыре частных файла: DXF/DWG-эталон, кириллица в слоях и виртуальный диск. Публичные тесты используют синтетическую геометрию без производственных чертежей. Необязательные частные проверки описаны в [матрице DXF](docs/DXF_SUPPORT.md), [руководстве DWG](docs/DWG_SUPPORT.md) и [описании проверки прорезей](docs/releases/v0.8.1.md).
