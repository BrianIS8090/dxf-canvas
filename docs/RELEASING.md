...
GitHub Actions запускает форматирование, 72 теста, Clippy и сборку EXE при изменениях `main`/`dev`, в предложениях изменений и по тегам `v*`. Версии действий закреплены идентификаторами коммитов; обычная сборка имеет только право чтения. Право записи выделено отдельному шагу подготовки релиза.
...
5. Дождитесь успешной сборки. Она создаст **черновик** GitHub Release с EXE и `SHA256SUMS.txt`.
6. Проверьте описание и файл, затем опубликуйте черновик в разделе Releases. Старые релизы и теги не перезаписывайте.
...
В `dist/release` появятся `DXF-Canvas-X.Y.Z-windows-x64.exe` и `SHA256SUMS.txt`. Каталог `dist` не хранится в Git: исполняемые файлы размещаются в Releases, а не в истории исходников.
...
```powershell
Get-FileHash -LiteralPath './DXF-Canvas-0.5.3-windows-x64.exe' -Algorithm SHA256
```
